...
Model extraction and thumbnail rendering continue in the background, with visible cards taking priority. A batch-capable RSX backend exports up to eight models from the current archive in parallel; Unity then renders their thumbnails on its main thread. Existing thumbnails remain cached on disk.
