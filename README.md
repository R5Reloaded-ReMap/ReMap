...
Model extraction and thumbnail rendering continue in the background, with visible cards taking priority. A batch-capable RSX backend exports up to eight models from the current archive in parallel. While Unity renders those thumbnails on its main thread, RSX prefetches the next bounded batch so neither stage has to remain idle. Existing thumbnails remain cached on disk.
