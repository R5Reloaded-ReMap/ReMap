...
The fork source is expected beside this repository in `../rsx`. `Tools/Build-ReMap.ps1` builds `rsx.sln` in Release/x64 when needed; its project writes `bin/Release/rsx.exe` and the `.remap-session-v1` compatibility marker. An alternate checkout can be supplied with `-RsxRoot`. ReMap detects the sibling path automatically while running in the Unity editor. The Windows build copies both files, the RSX license, and its required third-party notices beside `ReMap.exe`. The distributed app always resolves that root-level binary, so no RSX path or version setting is exposed.
