...
Build the Release x64 solution normally. It writes `bin/Release/rsx.exe` plus one `.remap-session` manifest containing the cumulative protocol version (`4`). ReMap copies both files beside its executable and derives every earlier capability from that version. It can still read legacy `.remap-session-vN` markers for older local builds, but new releases do not ship them. CAST/PNG formats are unchanged, so existing exports and thumbnails remain reusable.

Protocol v1 starts with REMAP_SESSION READY 1 (tab separated). LOAD carries absolute RPAK paths, EXPORT carries a 16-digit asset GUID and an 8-digit job ID, QUIT ends the process. V2 adds EXPORTBATCH, V3 adds EXPORTGEOMETRY and EXPORTBATCHGEOMETRY, and V4 raises bulk model exports to 64 entries and adds grouped texture repair. After a native crash during textured R5F export, ReMap restarts RSX with the model-only whitelist and retries the affected batch without material textures. The native exporter creates a distinct job directory so identical model basenames cannot overwrite each other. Unity publishes complete.txt only after export acknowledgment, exact LOD0 matching and CAST validation.
